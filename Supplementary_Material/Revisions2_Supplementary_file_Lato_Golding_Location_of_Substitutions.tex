...
\renewcommand\refname{Additional References} %change name of bib
...
For the most up to date Supplementary Material, please visit \url{www.github.com/dlato/Location_of_Substitutions_and_Bacterial_Arrangements}.
...
We ran multiple instances of \p with varying numbers of \ecol genomes.
These data points were connected using a locally estimated scatter plot smoothing method and confidence intervals.
From this data, we determined that increasing the number of genomes exponentially increases the run time of \p.
It becomes impractical to align more than 27 genomes with \p, as anything over that would take more than 24h to run.
The estimation computational run time to complete the alignment of 100 genomes would take over a month.
...
\begin{figure}[H]
	\begin{center}
		\includegraphics[width=0.44\textwidth]{./figs/ecoli_10KB_weighted_subs_nonpar_23Sep20.pdf}
...
\begin{figure}[H]
	\begin{center}
		\includegraphics[width=0.44\textwidth]{./figs/bass_10KB_weighted_subs_nonpar_22Sep20.pdf}
...
\begin{figure}[H]
	\begin{center}
		\includegraphics[width=0.44\textwidth]{./figs/strep_10KB_weighted_subs_nonpar_14Nov20.pdf}
...
\begin{figure}[H]
	\begin{center}
		\includegraphics[width=0.44\textwidth]{./figs/sinoC_10KB_weighted_subs_nonpar_22Sep20.pdf}
...
\begin{figure}[H]
	\begin{center}
		\includegraphics[width=0.44\textwidth]{./figs/pSymA_10KB_weighted_subs_nonpar_22Sep20.pdf}
...
\begin{figure}[H]
	\begin{center}
		\includegraphics[width=0.44\textwidth]{./figs/pSymB_10KB_weighted_subs_nonpar_22Sep20.pdf}
...
We have systematically removed/left out each taxa from the original substitutions analysis (as described in the Main Paper) this is a ``\textbf{L}eave \textbf{O}ne \textbf{O}ut'' (\textbf{LOO}) analysis.
...
The results from the chromosomes of \ecol and \smel (Table \ref{tab:leave_one_out}) indicate that removing any of the taxa from these analysis, results in the same overall conclusion, that the number of substitutions decreases with increasing distance from the origin of replication.
For the remaining replicons (\bass, \strep, \pa and \pb of \smel), majority of the LOO results mirror what was found in the main paper when all taxa were present in the analysis (Table \ref{tab:leave_one_out}).
...
This shift in the outgroup is the cause for the coefficient estimate changing sign when these particular taxa (\sliv 1362 CM001889 and \smel BL225C NC\_017324) are removed from the analysis.
...
A linear regression to determine how the average \dn, \ds, and $\omega$ values in various sections of the genome (10Kbp, 25Kbp, 50Kbp, 100Kbp, 200Kbp, and 400Kbp) changes with genomic position was performed  (Table \ref{tab:lin_reg_windows_selection}).
%All additional linear regression results (Table \ref{tab:lin_reg_windows_selection}) mirror the results from the linear regression on all \dn, \ds, and $\omega$ values and changes in genomic position (see the Main Paper results section for more information).
...
\printbibliography[title={Additional References},heading=bibintoc]
\end{document}
