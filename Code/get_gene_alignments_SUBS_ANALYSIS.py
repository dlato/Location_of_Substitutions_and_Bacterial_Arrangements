...
print("ord_rev", ord_rev)

sys.stderr.write("done dealing with gaps!\n")
...
#                            print("codon_pos nex_gene", codon_pos)
                            nex_gene_codon_pos = 0
                            alt_codon_pos = 0
                ...
                if (i >= s_array[j+1]-1 and i <= e_array[j+1]-1):
                    ...
                    if i == overlap_end: #if we are at the end of the overlap to reset alt_codon_pos for next overlap
                        ...
                        alt_codon_pos = 0
                        nex_gene_codon_pos = 0
        print("RECORDED codon pos", cod_array[i])
        print("RECORDED gene_name", gene_name_array[i])
...
sys.stderr.write("get genome classifications!\n")

##print("PUTTING GAPS AND GENE INFO TOGETHER\n\n")
...
for i in range(0,aln_len):
    ...
    print("overall_class", overall_class[i])
...
for t in range(0,num_taxa):
    ...
    for x in range(0, aln_len):
        ...
        if (o_class == 0):
            ...
            section_gene_comp[x] = gc_array[x]
        print("RECORDED section_name", section_name[x])
                ...
                if (site != gene_name[next_site]):
                    ...
                    if (start_v == 0): #first site of aln
                        ...
                        if (len(aln_start) == 0):
                            ...
                            print("gene_comp", gene_comp_arr[j])
                            print("class gene_comp", type(gene_comp_arr[j]))
                            print("class gene_comp", type(int(gene_comp_arr[j])))
                    else:
                        tmp_j = j +1
            ...
            if (site != gene_name[next_site]):
                ...
                if (start_v == 0): #first site of aln
                    ...
                    if (len(aln_start) == 0):
                        ...
                        print("class gene_comp", type(int(gene_comp_arr[j])))
        j += 1
    print("LENGTH OF ALN_START", len(aln_start))
    print("LENGTH OF ALN_END", len(aln_end))
...
for i in gene_comp_list:
    print("gene_comp_lists len")
    print(len(i))
...
ref_aln_start = aln_start_list[ref_taxa]
ref_aln_end = aln_end_list[ref_taxa]
current_gene = []
gene_count = -1
q = -1
print("LENGTH OF GENE CLASS ARR", len(gene_class_list[ref_taxa]))
print("LENGTH OF ref_aln_start", len(ref_aln_start))
#going through each section of the genes. this includes coding,
#non-coding, and misc sections. and these can be broken down many many
#times to be separated by gaps and mis-aligning overall class of the
#nucleotide, so it makes sense that there are a lot of sections
for i in range(0,len(ref_aln_start)):
    print("-----------")
    print("i", i)
    print("aln pos:", ref_aln_start[i], "-", ref_aln_end[i])
    non_div = 0
    gene_gapped = 0
    gene_len_arr = gene_len_list[ref_taxa]
    ...
    print("gene_class_unique_pos[i]", gene_class_unique_pos[i])
    ...
    for j in range(0,num_taxa):
        ...
        print(">TEST",taxa_names[j],":",genome_pos_arr[int(ref_aln_start[i])],"-",genome_pos_arr[int(ref_aln_end[i])],"<")
        print("gene comp", gene_comp)
    ...
    cod_file.write("\n=\n")
    non_cod_file.write("\n=\n")
    #checking if the gene we are looking at is in our unique class list
